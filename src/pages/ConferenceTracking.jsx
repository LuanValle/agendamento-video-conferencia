...
const SEARCH_OPTIONS = [
  ...
  { value: 'nome', label: 'Nome da videoconferência', placeholder: 'Digite o nome da videoconferência' },
  { value: 'responsavel', label: 'Responsável', placeholder: 'Digite o nome do responsável' },
...
function ConferenceTracking() {
  ...
  const runSearch = async ({ type, value, targetPage }) => {
    try {
      ...
      const response = await fetch(`/api/acompanhamento?${params}`)
      const result = await response.json()

      if (!response.ok) {
        throw new Error(result.error || 'Não foi possível realizar a busca.')
      }
      ...
      <main className="tracking-page">
        ...
        <section className="tracking-panel">
          <div className="tracking-heading">
            ...
            <h1>Acompanhar videoconferências</h1>
            <p>Consulte o andamento de uma solicitação. Por segurança, links de acesso não são exibidos.</p>
          </div>
          ...
          {!loading && hasSearched && !error && results.length === 0 && (
            <div className="tracking-empty" role="status">
              Nenhuma videoconferência encontrada para essa busca.
            </div>
          )}
            ...
            <section className="tracking-results" aria-live="polite">
              <div className="tracking-results-heading">
                <div>
                  ...
                  <p>Exibindo até 10 videoconferências por página.</p>
                </div>
                  ...
                  <tbody>
                    {results.map((conference) => (
                      <tr key={conference.id}>
                        ...
                        <td data-label="Local">{conference.local || 'Não informado'}</td>
