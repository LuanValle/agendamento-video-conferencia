const SYSTEM_EMAIL = 'com9dn.sti@marinha.mil.br'
...
export const buildCallTicketText = (conference) => {
  ...
  return [
    'ACD DCTIMBOTEC N\u00ba 32/001/2018, SOL cria\u00e7\u00e3o de reuni\u00e3o virtual utilizando plataforma Webex com os seguintes dados:',
    `ALFA - ${conference.responsible || 'NOME DO SOLICITANTE'}`,
    `BRAVO - ${responsibleEmail}, ${SYSTEM_EMAIL}`,
    `CHARLIE - ${buildDateLine(conference, endTime)}`,
    `DELTA - ${duration}`,
    `ECHO - ${conference.name || 'TITULO DA REUNIAO'}`,
  ].join('\n')
}
